...
{
    public async Task<GetCandidatesFormsByFilterResponse> Handle(GetCandidatesFormsByFilterQuery request, CancellationToken cancellationToken)
    {
        ...
        if(!string.IsNullOrWhiteSpace(request.Category))
            query = query
                .Where(form => form.CandidateForm.ProfessionName.ToLower().Equals(request.Category));
        
        if(!string.IsNullOrWhiteSpace(request.City))
            query = query
                .Where(form => form.CandidateForm.PlaceAddress.ToLower().Equals(request.City));
